...
def main():
    ...
    out_file = f"{common}all_hist.png"
    ...
    fig.suptitle(f"SOEM の送信開始から受信完了までの各処理区間の処理時間ヒストグラム (ビン幅: {BIN_WIDTH_US}us, 通信回数: {len(user_data)})", fontsize=14)

    max_freq = max([np.histogram(sample, bins=50)[0].max() for sample, _ in datasets])
