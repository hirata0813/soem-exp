...
// USER CODE
int pdo_datatype_addr;
int pdo_bitlength_addr;
...
int si_siiPDO(uint16 slave, uint8 t, int mapoffset, int bitoffset)
{
   ...
   {
      a = PDO->Startpos;
      printf("[INFO] PDO Startpos: 0x%4.4X\n", a);
      ...
      do
      {
         ...
         if (PDO->SyncM[PDO->nPDO] < EC_MAXSM) /* active and in range SM? */
         {
            ...
            for (er = 1; er <= e; er++)
            {
               ...
               obj_name = ecx_siigetbyte(&ctx, slave, a++);
               printf("[INFO] Data Type address: 0x%4.4X\n", a);
               obj_datatype = ecx_siigetbyte(&ctx, slave, a++);
               printf("[INFO] Bit Length address: 0x%4.4X\n", a);
...
void slaveinfo(char *ifname)
{
   ...
   if (ecx_init(&ctx, ifname))
   {
      ...
      if (ecx_config_init(&ctx) > 0)
      {
         ...
         {
            // print_slaveinfo(&(ctx.slavelist[cnt]));
            ...
            {
               printf(" FMMU%1d offset:%8.8x len:%4d Lsb:%d Leb:%d Ps:%4.4x Psb:%d Ty:%2.2x Act:%2.2x\n", j,
            ...
            if (printMAP)
            {
               if (ctx.slavelist[cnt].mbx_proto & ECT_MBXPROT_COE) {
                  printf("[INFO] print SDO mapping from COE\n"); 
                  si_map_sdo(cnt);
               }
               else {
                  printf("[INFO] print SII mapping\n");
                  si_map_sii(cnt);
               }
            }
         }
...
void reconfig_pdo_mapping(void)
{
   int i, j, nSM;
   int outputs_bo = 0, inputs_bo = 0;
   int Tsize;

   printf("Reconfiguring PDO mapping\n");
   for (i = 1; i <= ctx.slavecount; i++)
   {
      ...
      for (nSM = 0; nSM < EC_MAXSM; nSM++)
      {
         if (ctx.slavelist[i].SM[nSM].StartAddr > 0)
         {
            Tsize = ctx.slavelist[i].SM[nSM].SMlength * 8;
            if (nSM == 2) // outputs
               outputs_bo += Tsize;
            if (nSM == 3) // inputs
               inputs_bo += Tsize;
   ...
   printf("Outputs: %d bits Inputs: %d bits\n", outputs_bo, inputs_bo);
}
...
int main(int argc, char *argv[])
{
   ...
   if (argc > 1)
   {
      ...
      slaveinfo(ifbuf);
      printf("------------------------------------------------------\n");
      print_slaveinfo(&ctx.slavelist[1]);
   }
   else
   {
   ...
   printf("[LOG] reconfig PDO mapping\n");
