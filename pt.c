...
int main(int argc, char *argv[])
{
  ...
  if (fieldbus_start(&fieldbus))
  {
    ...
    // printf("[INFO] CPU frequency: %f Hz\n", cpu_hz);
    ...
    for (i = 1; i <= repeat_cnt; ++i)
    {
      ...
      uint64_t diff_clock_poll_recv = clocks[4] - clocks[2];
      uint64_t diff_clock_recv = clocks[recv_end] - clocks[poll_end];
      uint64_t diff_clock_total = clocks[recv_end] - clocks[send_start];

      // Unix timestamp
      logfile_printf("%lld,%lld,%lld,%lld\n", diff_clock_send, diff_clock_poll_recv, diff_clock_recv, diff_clock_total);
